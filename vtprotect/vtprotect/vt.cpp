...
EXTERN_C void NTAPI SetupVMCS(ULONG64 _rsp,ULONG64 _rip)//可以加个ucpuid参数
{
	_EFLAGS uEflags;
	...
	*((PULONG64)&uEflags) = GetRflags();
	...
	Vmx_VmWrite(GUEST_SYSENTER_EIP, ReadMsr(MSR_IA32_SYSENTER_EIP)); // KiFastCallEntry
																		 ...
																		 //Vmx_VmWrite(GUEST_RIP, GetGuestReturn());// 指定vmlaunch客户机的入口点 这里我们让客户机继续执行加载驱动的代码
	...
	Vmx_VmWrite(HOST_RSP, ((ULONG64)g_VMXCPU[uCPUID].pHostEsp) + 0x1FFF);//8KB 0x2000
																		 //Vmx_VmWrite(HOST_RIP,(ULONG64)&VMMEntryPoint);//这里定义我们的VMM处理程序入口
